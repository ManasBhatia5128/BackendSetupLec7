...
9. return response
  ...
  await User.create({ // time required in creation
    fullName,
    username,
    email,
    password,
  ...
  const createdUser = await User.findById(username._id).select("-password -refreshToken") // _id mongoDB bana leta hai har field ki
